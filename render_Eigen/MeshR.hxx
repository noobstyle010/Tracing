////////////////////////////////////////////////////////////////////
//
// $Id: MeshR.hxx 2021/06/13 15:14:46 kanai Exp $
//
...
public:
  ...
  //  Eigen::Vector3f& point( int i ) { return Eigen::Vector3f( points_[3*i], points_[3*i+1], points_[3*i+2] ); };
  //  Eigen::Vector3f& normal( int i ) { return Eigen::Vector3f( normals_[3*i], normals_[3*i+1], normals_[3*i+2] ); };
  //  Eigen::Vector3f& texcoord( int i ) { return Eigen::Vector3f( texcoords_[3*i], texcoords_[3*i+1], texcoords_[3*i+2] ); };
  //  Eigen::Vector3f& color( int i ) { return Eigen::Vector3f( colors_[3*i], colors_[3*i+1], colors_[3*i+2] ); };
  ...
  void setPoint( int i, Eigen::Vector3f& p ) {
    points_[i]   = p.x();
    points_[i+1] = p.y();
    points_[i+2] = p.z();
  };
  ...
  void setNormal( int i, Eigen::Vector3f& p ) {
    normals_[i]   = p.x();
    normals_[i+1] = p.y();
    normals_[i+2] = p.z();
  };
  ...
  void setTexcoord( int i, Eigen::Vector3f& p ) {
    texcoords_[i]   = p.x();
    texcoords_[i+1] = p.y();
    texcoords_[i+2] = p.z();
  };
  ...
  void getFacePoints( unsigned int face_id, Eigen::Vector3f& p0, Eigen::Vector3f& p1, Eigen::Vector3f& p2 ) {
    ...
    p0 << points_[ i0 ], points_[ i0 + 1 ], points_[ i0 + 2 ];
    p1 << points_[ i1 ], points_[ i1 + 1 ], points_[ i1 + 2 ];
    p2 << points_[ i2 ], points_[ i2 + 1 ], points_[ i2 + 2 ];
  };
  ...
  void calcNormal( Eigen::Vector3f& p0, Eigen::Vector3f& p1, Eigen::Vector3f& p2, Eigen::Vector3f& nrm ) {
  ...
  void calcNormald( Eigen::Vector3d& p0, Eigen::Vector3d& p1, Eigen::Vector3d& p2, Eigen::Vector3d& nrm ) {
  ...
  float calcFaceArea( Eigen::Vector3f& p0, Eigen::Vector3f& p1, Eigen::Vector3f& p2 ) {
    Eigen::Vector3f v1( p1 - p0 );
    Eigen::Vector3f v2( p2 - p0 );
    //Eigen::Vector3f a;
  ...
  void normalize( Eigen::Vector3f& center, float maxlen ) {
    if ( isNormalized_ ) return;
  ...
  void getTransScale( Eigen::Vector3f& vmin, Eigen::Vector3f& vmax, Eigen::Vector3f& cen,
                      Eigen::Vector3f& len, float* maxlen ) {
    for ( unsigned int i = 0; i < points_.size(); i += nXYZ )
      {
        float x = points_[i]; 
        float y = points_[i+1]; 
  ...
  Eigen::Vector3f& vmin() { return vmin_; };
  Eigen::Vector3f& vmax() { return vmax_; };
  Eigen::Vector3f& length() { return length_; };
  ...
  void createFaceNormals() {
    ...
    for ( unsigned int i = 0; i < numFaces(); ++i )
      {
        Eigen::Vector3f nrm;
        Eigen::Vector3f p1( points_[ nXYZ * indices_[ TRIANGLE * i ] ],
                            points_[ nXYZ * indices_[ TRIANGLE * i ] + 1 ],
                            points_[ nXYZ * indices_[ TRIANGLE * i ] + 2 ] );
        Eigen::Vector3f p2( points_[ nXYZ * indices_[ TRIANGLE * i + 1 ] ],
                            points_[ nXYZ * indices_[ TRIANGLE * i + 1 ] + 1 ],
                            points_[ nXYZ * indices_[ TRIANGLE * i + 1 ] + 2 ] );
        Eigen::Vector3f p3( points_[ nXYZ * indices_[ TRIANGLE * i + 2 ] ],
    ...
    for ( unsigned int i = 0; i < indices_.size(); i += TRIANGLE )
      {
        ...
        // Eigen::Vector3f p0 ( (double) points_[ nXYZ * id0 ], 
        //                      (double) points_[ nXYZ * id0 + 1 ],
        //                      (double) points_[ nXYZ * id0 + 2 ] );
        // Eigen::Vector3f p1 ( (double) points_[ nXYZ * id1 ], 
        //                      (double) points_[ nXYZ * id1 + 1 ],
        //                      (double) points_[ nXYZ * id1 + 2 ] );
        // Eigen::Vector3f p2 ( (double) points_[ nXYZ * id2 ], 
        //                      (double) points_[ nXYZ * id2 + 1 ],
        //                      (double) points_[ nXYZ * id2 + 2 ] );

        Eigen::Vector3f nrm = Eigen::Vector3f( fnormals_[ nXYZ * f_id ],
                                               fnormals_[ nXYZ * f_id + 1 ],
                                               fnormals_[ nXYZ * f_id + 2 ] );
    ...
    for ( unsigned int i = 0; i < indices_.size(); i += TRIANGLE )
      {
        ...
        Eigen::Vector3f p0 ( points_[ nXYZ * id0 ], 
                     points_[ nXYZ * id0 + 1 ],
                     points_[ nXYZ * id0 + 2 ] );
        Eigen::Vector3f p1 ( points_[ nXYZ * id1 ], 
                     points_[ nXYZ * id1 + 1 ],
                     points_[ nXYZ * id1 + 2 ] );
        Eigen::Vector3f p2 ( points_[ nXYZ * id2 ], 
                     points_[ nXYZ * id2 + 1 ],
                     points_[ nXYZ * id2 + 2 ] );
    ...
    for ( unsigned int i = 0; i < numPoints(); ++i )
      {
        ...
        Eigen::Vector3f nrm( normals_[ nXYZ * i ], normals_[ nXYZ * i + 1 ], normals_[ nXYZ * i + 2 ] );
        nrm.normalize();
  ...
  void createVertexNormalsWithSF() {
    ...
    std::vector< std::vector<Eigen::Vector3f> > vnrm( numPoints() );
    ...
    for ( unsigned int i = 0; i < numFaces(); ++i )
      {
        ...
        // 平面の法線
        Eigen::Vector3f nf( fnormals_[ nXYZ * i ], 
                            fnormals_[ nXYZ * i + 1 ], 
                            fnormals_[ nXYZ * i + 2 ] );
            ...
            for ( unsigned int k = 0; k < n_vn[id[j]].size(); ++k )
              {
                ...
                // 格納されているグループの法線
                Eigen::Vector3f& nf0 = vnrm[id[j]][k];
#if 0
...
#endif
                // 閾値以内だったら法線を平均する
                if ( angleT<float>( nf, nf0 ) < th )
                  {
                    unsigned int kvn = n_vn[id[j]][k];
                    nf0 *= (float) kvn;
                    nf0 += nf;
                    nf0 *= (1.0f / (float) (kvn+1));
                    ++(n_vn[id[j]][k]);
  ...
  void getBBox( Eigen::Vector3f& vmax, Eigen::Vector3f& vmin ) {

    for ( unsigned int i = 0; i < points_.size(); i += 3 )
      {
        Eigen::Vector3f p( points_[i], points_[i+1], points_[i+2] );
        if ( i )
          {
...
private:
  ...
  // for object
  Eigen::Vector3f vmin_;
  Eigen::Vector3f vmax_;
  Eigen::Vector3f center_;
  Eigen::Vector3f length_;
  float   maxlen_;
